...
import { getStoredWallet, getWalletBalance, truncateAddress } from "@/src/utils/walletService";
...
export default function TopBar({
  ...
  useEffect(() => {
    const walletAddr = getStoredWallet();
  ...
  useEffect(() => {
    ...
    const tBalance = setInterval(() => {
      if (walletAddress) {
        getWalletBalance(walletAddress).then((bal) => setWalletBalance(bal));
      }
    }, 15000);
    
    return () => { 
      clearInterval(tTime); 
      clearInterval(tBlock); 
      clearInterval(tAgo);
      clearInterval(tBalance);
    };
  }, [walletAddress]);
    ...
    <div style={{ marginBottom: 28, flexShrink: 0 }}>
      ...
      >
        ...
        >
          {walletAddress ? (
            <>
              <span style={{ fontFamily: "monospace", fontSize: 8, color: "rgba(0,255,209,0.5)", letterSpacing: "0.08em" }}>
                WALLET
              </span>
              ...
              <span style={{ 
                fontFamily: "monospace", 
                fontSize: 10, 
                color: balanceLoading ? "rgba(255,255,255,0.3)" : "#00FFD1",
                fontWeight: 600,
                marginLeft: 4,
              }}>
                {balanceLoading ? "..." : walletBalance.toFixed(2)} XLM
              </span>
            </>
          ) : (
            <>
