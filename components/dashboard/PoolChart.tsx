...
export default function PoolChart({ pair = "XLM_USDC" }: { pair?: string }) {
  ...
  const [liquidityPools, setLiquidityPools] = useState<Array<{ id: string; type: string; feeBps: number; totalShares: string; reserves: Array<{ asset: string; amount: string }> }>>([]);
  ...
  const loadMarket = useCallback(async (tf: TF) => {
    setError(null);
    try {
      const [ohlc, marketStats, poolSnapshot, pools] = await Promise.all([
        fetchOHLC(tf, pair).catch(() => [] as OHLCCandle[]),
        fetchMarketStats(pair).catch(() => null),
        fetchPoolSnapshot().catch(() => null),
        fetchLiquidityPools().catch(() => []),
      ]);
      setCandles(ohlc);
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 4 }}>
            <span className="font-display" style={{ fontSize: 30, color: "#F0F0F0" }}>
              {loading ? "--" : (snapshot?.usdcPerXlm ?? currentPrice)?.toFixed(4) ?? "--"}
            </span>
          ...
          <div style={{ marginTop: 4, fontFamily: "Inter,sans-serif", fontSize: 12, color: "rgba(255,255,255,0.45)" }}>
            {liquidityPools.length > 0
              ? `Stellar Liquidity Pools: ${liquidityPools.length} pools available`
              : "Loading liquidity pools from Stellar testnet..."}
          </div>
        </div>
