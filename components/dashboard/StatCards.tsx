...
import type { PoolInfo, UserPool } from "@/lib/mockData";
import { estimateXlmFromShares } from "@/src/utils/poolService";
...
export default function StatCards({ pool, user, lending, loading = false, errors }: Props) {
  ...
  const refreshKey = `${pool.totalDeposits}-${user.shares}-${netAuraPoints}`;
  const poolCount = useCounter(pool.totalDeposits, 1000, loading ? 0 : refreshKey.length + pool.totalDeposits);
  const sharesCount = useCounter(user.shares, 1000, loading ? 0 : refreshKey.length + user.shares);
  const auraCount = useCounter(netAuraPoints, 900, loading ? 0 : refreshKey.length + netAuraPoints);
  const shareXlmEstimate = estimateXlmFromShares(user.shares, pool);
  ...
  useEffect(() => {
    if (loading) return;
    const t = setTimeout(() => {
      setBarW(Math.min((pool.totalDeposits / 100000) * 100, 100));
      setAuraBarW(Math.min((netAuraPoints / 100) * 100, 100));
    }, 600);
    return () => clearTimeout(t);
  }, [loading, netAuraPoints]);
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr 0.9fr 0.9fr", gap: 14 }}>
      ...
      <Card delay={0}>
        ...
        <div style={{ fontSize: 28, fontWeight: 700, color: "#F0F0F0", marginTop: 10, letterSpacing: "-0.03em", lineHeight: 1 }}>
          {loading ? (
            ...
            <>
              ...
              <span style={{ fontSize: 13, fontWeight: 400, color: "rgba(255,255,255,0.4)", marginLeft: 4 }}>XLM</span>
            </>
          )}
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 6 }}>
          <span style={{ color: "#00FFD1", fontSize: 11 }}>↑</span>
          <span style={{ fontFamily: "Inter,sans-serif", fontSize: 12, color: "#00FFD1" }}>Live pool</span>
        </div>
        <div style={{ marginTop: 14 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
            <span style={{ fontFamily: "monospace", fontSize: 9, color: "rgba(255,255,255,0.25)", letterSpacing: "0.1em" }}>SHARES</span>
            <span style={{ fontFamily: "monospace", fontSize: 9, color: "rgba(0,255,209,0.6)" }}>{pool.totalShares}</span>
          </div>
      ...
      <Card delay={80}>
        ...
        <div style={{ fontFamily: "Inter,sans-serif", fontSize: 12, color: "rgba(255,255,255,0.35)", marginTop: 6 }}>
          {loading ? "Loading..." : `≈ ${shareXlmEstimate.toFixed(4)} XLM`}
        </div>
      ...
      <Card delay={160}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          ...
          <AuraArc pts={netAuraPoints} />
        </div>
        <div style={{ fontSize: 28, fontWeight: 700, color: "#FFB347", marginTop: 6, letterSpacing: "-0.03em", lineHeight: 1 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 6 }}>
          {lending.unsecuredEligible ? (
            ...
            <span style={{ fontFamily: "Inter,sans-serif", fontSize: 12, color: "rgba(255,255,255,0.3)" }}>
              Need {lending.minAuraPointsForUnsecured} pts
            </span>
          )}
        </div>
      ...
      <Card delay={240}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          ...
          <svg width="18" height="18" viewBox="0 0 18 18" fill="none"
            style={{ opacity: 0.25 }}>
            <circle cx="9" cy="9" r="7" stroke={lending.activeLoan ? "#FFB347" : "rgba(255,255,255,0.4)"} strokeWidth="1.3" />
            <path d="M9 5v4.5l2.5 2.5" stroke={lending.activeLoan ? "#FFB347" : "rgba(255,255,255,0.4)"} strokeWidth="1.3" strokeLinecap="round" />
          </svg>
        </div>
        {loading ? (
          ...
          <>
            <div style={{ fontSize: 28, fontWeight: 700, color: "#F0F0F0", marginTop: 10, letterSpacing: "-0.03em", lineHeight: 1 }}>
              {(lending.principal / 10_000_000).toFixed(4)}
              <span style={{ fontSize: 13, fontWeight: 400, color: "rgba(255,255,255,0.4)", marginLeft: 4 }}>XLM</span>
            </div>
            <div style={{ fontFamily: "Inter,sans-serif", fontSize: 12, color: "#FFB347", marginTop: 6 }}>
              Active loan
            </div>
          </>
        ) : (
          <>
            <div style={{ fontSize: 24, fontWeight: 700, color: "rgba(255,255,255,0.2)", marginTop: 10, letterSpacing: "-0.03em" }}>
              None
            </div>
